...
export const SubtitleCard = (props:Props) => {
  ...
  const handleSetThumbnail = useCallback(async() => {
    ...
    const {error} = await client.from('manga-title').upsert(title)
    if (error) {
  ...
  const handleSave = useCallback(async() => {
    ...
    const {error} = await client.from('manga-subtitle')
      .upsert({
              ...
              <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform border border-gray-300 shadow-xl bg-gray-50 rounded-xl">
                ...
                <div className="mx-4 mt-4 bg-blue-50">
                  <Disclosure>
                    {({open}) => (
                      <>
                        <Disclosure.Button className="flex justify-between w-full px-4 py-2 font-medium text-left text-blue-500 bg-blue-100 rounded-lg hover:bg-blue-200 focus:outline-none focus-visible:ring focus-visible:ring-blue-500 focus-visible:ring-opacity-75">
